/* Portable SHA-256 (FIPS 180-4). Processes full 64-byte blocks directly from
 * the input and pads only the final block(s), so it never copies the whole
 * message — important for multi-hundred-KB images on a constrained device.
 */
...
static void transform(uint32_t h[8], const uint8_t block[64]) {
    uint32_t w[64];
    for (int i = 0; i < 16; ++i) {
...
void sd_sha256(const uint8_t *data, size_t len, uint8_t out[32]) {
    ...
    for (size_t i = 0; i < full; ++i) {
        transform(h, data + i * 64);
    }

    /* Final block(s): remainder + 0x80 + zero pad + 64-bit big-endian bit len. */
    uint8_t tail[128];
